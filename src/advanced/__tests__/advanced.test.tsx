...
describe("장바구니 시나리오 테스트", () => {
  ...
  it("초기 상태: 상품 목록이 올바르게 그려졌는지 확인", () => {
    ...
    // 첫 번째 상품 확인
    expect(select.children[1].value).toBe("p1");
    expect(select.children[1].textContent).toBe("상품1 - 10000원");
    expect(select.children[1].disabled).toBe(false);

    // 재고 없는 상품 확인 (상품4)
    expect(select.children[4].value).toBe("p4");
    expect(select.children[4].textContent).toBe("상품4 - 15000원");
    expect(select.children[4].disabled).toBe(true);
  });
