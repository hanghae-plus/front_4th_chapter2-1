...
const getCalcCartItem = (cartItem) => {
  ...
  let qty = parseInt(cartItem.querySelector("span").textContent.split("x ")[1]);
...
function quantityChange(target, itemElement, product) {
  ...
  if (newQty > 0 && newQty <= currentQty + product.qty) {
...
const handleClickQuantityChange = () => {
  ...
  if (
    ...
    if (target.classList.contains("quantity-change")) {
      quantityChange(target, itemElement, product);
    } else if (target.classList.contains("remove-item")) {
      removeItem(itemElement, product);
    }
