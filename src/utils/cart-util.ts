...
import { CartWithElements, Product } from '../interface/cart';
import { createChildElement, createStyledElement } from './element-util';

export function cartService() {
  // 장바구니 총액 계산 함수
  function calculateCartTotal(cart: CartWithElements) {
    cart.totalAmt = 0;
    cart.itemCnt = 0;
  ...
  // 대량 구매 할인 적용 함수
  function calculateDiscountRate(subTotal: number, cart: CartWithElements) {
    if (cart.itemCnt >= 30) {
  ...
  // 보너스 포인트 렌더 함수
  function renderBonusPoints(cart: CartWithElements) {
  ...
  // 재고 상태 업데이트 함수
  function updateStockStatus(cart: CartWithElements) {
  ...
  // 장바구니에 제품을 추가하는 함수
  function addCartProduct(cart: CartWithElements, product: Product) {
  ...
  function addNewCartProduct(cart: CartWithElements, product: Product) {
  ...
  function updateCartTotal(cart: CartWithElements) {
